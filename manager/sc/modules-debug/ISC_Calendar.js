...
addEvent : function (startDate, endDate, name, description, otherFields, ignoreDataChanged, laneName) {
    ...
    var _finish = function (dsResponse, data, dsRequest) {
        ...
        if (self.logIsDebugEnabled("calendar")) {
            self.logDebug('event added:' + self.echo(newEvt), 'calendar');
        }
...
_getNewEventWindow : function (event) {
    ...
    var canRemove = this.canRemoveEvent(event);
...
getCellDate : function (rowNum, colNum) {
    ...
    if (this.dayViewSelected()) {
        ...
        var grid = this.timelineView,
            frozenCount = grid.frozenFields ? grid.frozenFields.length : 0,
            body = colNum < frozenCount ? grid.frozenBody : grid.body,
            fieldIndex = colNum < frozenCount ? colNum : colNum - frozenCount,
